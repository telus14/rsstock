...
<meta charset="utf-8"/>
    ...
    <body>
    <h1 class="title"><b>ImpressoraS</b><br> </h1>
    <h3 class="esto"></h3>
    <div class="b1">
        <input type="submit" value="ENTRADA Printers" 
        onclick="location.href='entraimp.php'">
        </div>
        
</form>
    <div class="div2">
    <form action="impsai.php" method="POST">
        <label>Saida</label><br>
    <input id="ext" name="numero" type="number" placeholder="Num Impressora"><br>
    <input name="cliente" type="text" value="<?php
    $sql = 'SELECT client_f FROM impmov ORDER BY id DESC LIMIT 1';
...
<!--BOTAO PARA PESQUISA   ]
 <form action="pbusca.php" method="POST">
        <input name="busc" placeholder="home or not?" type="text">
        <button name="busca" type="submit">Try</button>
    </form>
-->
<div class="menu">
    ...
    $query_iestoque = "SELECT COUNT(id) AS imphome FROM imp WHERE estoque=1";
    $result_imp = $conn->prepare($query_iestoque);
    $result_imp->execute();

    $row_produto = $result_imp->fetch(PDO::FETCH_ASSOC);
    echo "Total Home:  " . $row_produto['imphome'] . "<br>"
    ?>
    </div>

    <div class="div4">
    <h3><a href="lisimp.php">__Estoque__</a></h3>
    </div>

    </div>
<div class="exce">
<a href="impexcell.php">Gerar Movimento!</a><br>
</div>
