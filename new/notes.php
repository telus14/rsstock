...
<meta charset="utf-8"/>
    ...
    <body>
    <h1 class="title"><b>__NotesS__</b><br> </h1>
    <h3 class="esto"></h3>
    <div class="b1">
        <input type="submit" value="ENTRADA Notes" 
        onclick="location.href='entranot.php'">
        </div>


    <div class="div2">
    <form action="notsai.php" method="POST">
        <label>Saida</label><br>
    <input id="ext" name="numero" type="number" placeholder="Num Not"><br>
    <input name="cliente" type="text" value="<?php
    $sql = 'SELECT client_f FROM notmov ORDER BY id DESC LIMIT 1';
...
<?php
    ...
    $query_testoque = "SELECT COUNT(id_note) AS nhome FROM note WHERE estoque=1";
    $result_tablet = $conn->prepare($query_testoque);
    $result_tablet->execute();

    $row_produto = $result_tablet->fetch(PDO::FETCH_ASSOC);
    echo "Total Home:  " . $row_produto['nhome'] . "<br>"
    
    ?>
    </div>
    <div class="div4">
    <h3><a href="lisnot.php">__Estoque__</a></h3>
</div>
<div class="exce">
<a href="gexcell.php">Gerar Movimento!</a><br>
</div>
...
<?php
    ...
    <form method="POST" action="">
		...
		if($SendPesqUser){
			...
			$result_usuario = "SELECT * FROM note WHERE numero = '$numero'";
			$resultado_usuario = mysqli_query($conexao, $result_usuario);
			while($row_usuario = mysqli_fetch_assoc($resultado_usuario)){
				echo "> " . $row_usuario['numero'] . " - ";
