...
<head>
    ...
    <h3 class="esto"></h3>
    <div class="b1">
        <input type="submit" value="ENTRADA ALL" 
        onclick="location.href='entraall.php'">
        </div>
    
    <div class="div2">
    <form action="alle.php" method="POST">
        <label>SAIDA</label><br>
    <input id="ext" name="numero" type="number" placeholder="Num All"><br>
    <input name="cliente" type="text" value="<?php
    $sql = 'SELECT client_f FROM allmov ORDER BY id DESC LIMIT 1';
    $sql_result = $conn->prepare($sql);
    $sql_result->execute();
    $row_produto = $sql_result->fetch(PDO::FETCH_ASSOC);
    echo $row_produto['client_f'] ?>"><br>
...
<?php
    ...
    $query_testoque = "SELECT COUNT(id_all) AS ahome FROM allio WHERE estoque=1";
    $result_tablet = $conn->prepare($query_testoque);
    $result_tablet->execute();

    $row_produto = $result_tablet->fetch(PDO::FETCH_ASSOC);
    echo "   Total Home:  " . $row_produto['ahome'] . "<br>"
    
    ?>
    </div>
    <div class="div4">
    <h3><a href="lisall.php">__Estoque__</a></h3>
    </div>

    </div>
    <div class="exce">
    <a href="allexcell.php">Gerar Movimento!</a><br>
    </div>
    ...
    <form method="POST" action="">
		...
		if($SendPesqUser){
			...
			$result_usuario = "SELECT * FROM allio WHERE numero = '$numero'";
			$resultado_usuario = mysqli_query($conexao, $result_usuario);
			while($row_usuario = mysqli_fetch_assoc($resultado_usuario)){
				echo "ALL: " . $row_usuario['numero'] . "<br>";
				echo "Mod : " . $row_usuario['modelo'] . "<br>";
